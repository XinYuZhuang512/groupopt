...
DATA_ROOT="${DATA_ROOT:-/root/autodl-tmp/groupopt-data/paper_summary_gradient_am_tsp50_v1}"
...
EXIT_FILE="${EXIT_FILE:-${DATA_ROOT}/paper_summary_gradient_am_tsp50_v1.exit}"
MAIN_LOG="${MAIN_LOG:-${DATA_ROOT}/paper_summary_gradient_am_tsp50_v1.log}"
...
MAIN_ROOT="${MAIN_ROOT}" ABLATION_ROOT="${ABLATION_ROOT}" \
PYTHON_BIN="${PYTHON_BIN}" \
  bash "${PROJECT_ROOT}/experiments/paper/run_summary_gradient_am_tsp50.sh" \
  >"${MAIN_LOG}" 2>&1
