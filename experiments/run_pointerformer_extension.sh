...
run_one() {
  ...
  if [[ ! -f "${checkpoint}" ]]; then
    echo "missing Pointerformer checkpoint for ${mode}: ${checkpoint}" >&2
    return 1
  fi
  "${PYTHON_BIN}" experiments/train_pointerformer_experiment.py \
    --base-mode "${mode}" \
...
if [[ "${failed}" -ne 0 ]]; then
  echo "one or more Pointerformer extension runs failed" >&2
  exit 1
fi
