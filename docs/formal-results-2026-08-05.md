...
## Joint-action v3：四种子跨模型确认

v3 将原来分离的 tail/head 决策合并为一个合法边动作
`(tail, head)`。`joint_fixed` 与 `joint_free` 使用完全相同的参数与
scorer，只有框架层的 tail 合法性 mask 不同。每组训练 2,000 steps，
最佳 checkpoint 统一评估于同一批 10,000 个 TSP50 实例。

| 模型 | fixed 均值 | free 均值 | 相对改善 | seed 层面 95% CI | 胜局 |
| --- | ---: | ---: | ---: | ---: | ---: |
...
joint_free 在 3 个模型×4 个 seed 的 12/12 个配对中全部胜出。AM 和
GPN 的训练-seed 层面置信区间完全低于零；PtrNet 的上界仅略高于零，
但四个观测方向一致。这是当前首个稳定的短时程跨模型正向结果，
下一步应分阶段验证 10,000-step 长时程优势是否保留。
